...
<div class="row">
    ...
    <div class="col-md-12">
        <div class="form-group bmd-form-group">
            <label class="bmd-label-floating">{{trans("admin.cover_image")}}</label>
            @php $input = 'cover_image'; @endphp
            <input type="file" class="form-control @error($input) is-invalid @enderror" name="{{$input}}">
            @error($input)
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
    ...
    <div class="col-md-12">
        <div class="form-group bmd-form-group">
            <label class="bmd-label-floating">{{trans("admin.photos_more")}}</label>
            @php $input = 'photos[]'; @endphp
            <input type="file" class="form-control @error($input) is-invalid @enderror" name="{{$input}}" multiple>
            @error($input)
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
                </span>
